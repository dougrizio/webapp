...
def index():
    user = {'username': 'Male Oscar Winners'}
...
def record_view(oscar_id):
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM tblOscarsImport WHERE fldIndex=%s', oscar_id)
    result = cursor.fetchall()
...
def form_update_post(oscar_id):
    ...
    sql_update_query = """UPDATE tblOscarsImport t SET t.fldName = %s, t.fldYear = %s, t.fldAge = %s, t.fldName = 
...
def form_insert_post():
    ...
    sql_insert_query = """INSERT INTO tblOscarsImport (fldName,fldLat,fldLong,fldCountry,fldAbbreviation,fldCapitalStatus,fldPopulation) VALUES (%s, %s,%s, %s,%s, %s,%s) """
...
def form_delete_post(oscar_id):
    cursor = mysql.get_db().cursor()
    sql_delete_query = """DELETE FROM tblOscarsImport WHERE fldIndex = %s """
...
@app.route('/api/v1/cities/<int:oscar_id>', methods=['GET'])
...
@app.route('/api/v1/cities/<int:oscar_id>', methods=['PUT'])
def api_edit(oscar_id) -> str:
    resp = Response(status=201, mimetype='application/json')
    return resp


@app.route('/api/v1/cities/<int:oscar_id>', methods=['DELETE'])
